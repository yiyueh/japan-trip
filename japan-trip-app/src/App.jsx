    ...
    days: [
      {
        ...
        spots: [
          { id: 1,  time: "13:00", name: "大通公園",         cat: "景點", note: "札幌雪景最美",        done: false, budget: 0,    transit: "" },
          { id: 2,  time: "14:00", name: "札幌電視塔",       cat: "景點", note: "登頂 ¥1,000",         done: false, budget: 1000, transit: "徒步 2 分" },
          { id: 3,  time: "18:30", name: "元祖拉麵橫丁",     cat: "餐廳", note: "必吃奶油味噌拉麵",    done: false, budget: 1200, transit: "徒步 8 分" },
        ],
      },
      {
        date: "2026-03-30", label: "Day 2",
        spots: [
          { id: 4,  time: "09:30", name: "小樽運河",         cat: "景點", note: "早晨人少光線好",      done: false, budget: 0,    transit: "" },
          { id: 5,  time: "10:30", name: "小樽運河遊船",     cat: "體驗", note: "40分鐘 ¥1,800",       done: false, budget: 1800, transit: "徒步 1 分" },
          { id: 6,  time: "13:00", name: "音樂盒博物館",     cat: "景點", note: "三層夢幻音樂盒世界",  done: false, budget: 700,  transit: "徒步 15 分" },
        ],
      },
      {
        date: "2026-03-31", label: "Day 3",
        spots: [
          { id: 7,  time: "10:30", name: "洞爺湖（Klook）",  cat: "體驗", note: "湖畔散步＋溫泉",      done: false, budget: 5000, transit: "" },
          { id: 8,  time: "14:00", name: "洞爺湖溫泉旅館",   cat: "溫泉", note: "Lake View Toya 足浴", done: false, budget: 1500, transit: "巴士 5 分" },
        ],
      },
      {
        date: "2026-04-01", label: "Day 4",
        spots: [
          { id: 9,  time: "09:00", name: "北海道神宮",       cat: "景點", note: "莊嚴森林氛圍",        done: false, budget: 0,    transit: "" },
          { id: 10, time: "11:00", name: "中島公園",         cat: "景點", note: "春日散步",            done: false, budget: 0,    transit: "地鐵 10 分" },
          { id: 11, time: "18:00", name: "まさじん 成吉思汗",cat: "餐廳", note: "⭐4.9 必提前訂位！",  done: false, budget: 3500, transit: "地鐵 12 分" },
        ],
      },
      {
        date: "2026-04-02", label: "Day 5",
        spots: [
          { id: 12, time: "10:00", name: "AOAO水族館",       cat: "景點", note: "藝術感水族館，企鵝超萌", done: false, budget: 2200, transit: "" },
          { id: 13, time: "14:00", name: "狸小路商店街",     cat: "購物", note: "伴手禮＋藥妝大採購",  done: false, budget: 8000, transit: "徒步 5 分" },
        ],
      },
      {
        date: "2026-04-03", label: "Day 6",
        spots: [
          { id: 14, time: "09:00", name: "最後一碗拉麵",     cat: "餐廳", note: "元祖橫丁 早午餐",     done: false, budget: 1000, transit: "" },
          { id: 15, time: "12:00", name: "新千歲機場出發",   cat: "交通", note: "14:40 起飛",          done: false, budget: 0,    transit: "快速列車 36 分" },
        ],
      },
    ],
    budget: { total: 50000 },
...
// ─── SpotForm Modal ───────────────────────────────────────────────────────────
function SpotForm({ title, initial, onConfirm, onCancel }) {
  ...
  return (
    <div style={S.overlay} onClick={onCancel}>
      <div style={S.sheet} onClick={e => e.stopPropagation()}>
          ...
          {Object.keys(CAT_COLORS).map(cat => {
            ...
            return (
              <button key={cat} onClick={() => set("cat", cat)}
                style={{ padding: "5px 13px", borderRadius: 20, border: `1.5px solid ${active ? cc.dot : "#E5E7EB"}`, background: active ? cc.bg : "#F9FAFB", color: active ? cc.text : "#6B7280", fontSize: 12, fontWeight: active ? 700 : 400, cursor: "pointer", transition: "all 0.15s" }}>
                {cat}
              </button>
            );
          })}
        ...
        {/* Budget */}
        <label style={S.lbl}>💴 預算 (¥)</label>
        <input style={S.inp} type="number" placeholder="0" value={f.budget} onChange={e => set("budget", e.target.value)} />

        <button
          style={{ ...S.confirmBtn, opacity: f.name.trim() ? 1 : 0.5 }}
...
export default function App() {
  ...
  const [syncStatus, setSyncStatus] = useState({}); // { [tripId]: "syncing"|"ok"|"error" }
  const [joinCode, setJoinCode]     = useState("");
  const [joinError, setJoinError]   = useState("");
  const [joinLoading, setJoinLoading] = useState(false);
  const syncTimers = useRef({});

  // ── Save shared trip to Firebase ──
  const saveShared = useCallback(async (trip) => {
    if (!trip.shareCode) return;
    setSyncStatus(p => ({ ...p, [trip.id]: "syncing" }));
    try {
      await set(ref(db, `trips/${trip.shareCode}`), trip);
  ...
  // ── Poll shared trips every 15s ──
  useEffect(() => {
    const poll = setInterval(async () => {
      const sharedTrips = trips.filter(t => t.shareCode && t.joinedByMe);
      for (const t of sharedTrips) {
        try {
          const snap = await get(ref(db, `trips/${t.shareCode}`));
          if (snap.exists()) {
            const remote = snap.val();
            if (remote.updatedAt > (t.updatedAt || 0)) {
              setTrips(prev => prev.map(p => p.id === t.id ? { ...remote, id: t.id, joinedByMe: true } : p));
            }
          }
        } catch {}
      }
  ...
  async function joinTrip() {
    ...
    try {
      ...
      if (!snap.exists()) { setJoinError("找不到此邀請碼，請確認後重試"); setJoinLoading(false); return; }
      const remote = snap.val();
  ...
  async function refreshTrip(tripId) {
    ...
    try {
      const snap = await get(ref(db, `trips/${trip.shareCode}`));
      if (snap.exists()) {
        const remote = snap.val();
  ...
  const totalSpent = trip
    ? trip.days.flatMap(d => d.spots).reduce((s, sp) => s + (sp.done ? Number(sp.budget) : 0), 0)
    : 0;
  ...
  function handleAddTrip() {
    ...
    setTrips(p => [...p, { id: Date.now(), ...newTrip, budget: { total: Number(newTrip.budget) }, days }]);
  ...
  return (
    <div style={S.app}>
      ...
      <header style={S.header}>
        ...
        <span style={S.headerTitle}>{trip?.name}</span>
        ...
        <button style={{ ...S.iconBtn, marginLeft: -4 }} onClick={() => setModal("shareTrip")}>
          ...
          <div style={S.sheet} onClick={e => e.stopPropagation()}>
            ...
            <button style={{ ...S.confirmBtn, marginTop: 20 }} onClick={() => {
              ...
              setTrips(prev => prev.map(t => {
                ...
                return { ...t, startDate: editDates.startDate, endDate: editDates.endDate, days: newDays };
              }));
              setActiveDay(0);
              setEditDates(null);
            }}>確認更新</button>
                  ...
                  >
                    ...
                    <div style={S.cardBody} onClick={() => { setEditSpot(sp); setModal("editSpot"); }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        {sp.budget > 0 && <span style={{ fontSize: 12, color: "#059669", fontWeight: 600 }}>¥{Number(sp.budget).toLocaleString()}</span>}
                        <span style={{ fontSize: 10, color: "#D1D5DB" }}>點擊編輯 ›</span>
                      </div>
                    </div>
          ...
          <div>
            <div style={S.budgetHero}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ opacity: 0.8, fontSize: 13 }}>總預算</span>
                <span style={{ fontSize: 21, fontWeight: 800 }}>¥{trip.budget.total.toLocaleString()}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                <span style={{ opacity: 0.8, fontSize: 13 }}>已確認花費</span>
                <span style={{ fontSize: 16, fontWeight: 700 }}>¥{totalSpent.toLocaleString()}</span>
              </div>
              <div style={{ height: 6, background: "rgba(255,255,255,0.25)", borderRadius: 3, overflow: "hidden", marginBottom: 6 }}>
                <div style={{ height: "100%", background: "#fff", borderRadius: 3, width: `${Math.min((totalSpent / trip.budget.total) * 100, 100)}%`, transition: "width 0.5s" }} />
              </div>
              <span style={{ fontSize: 12, opacity: 0.75 }}>剩餘 ¥{(trip.budget.total - totalSpent).toLocaleString()}</span>
            </div>
            {trip.days.map((d, i) => {
              const dayTotal = d.spots.reduce((s, sp) => s + Number(sp.budget), 0);
              return (
                <div key={i} style={{ background: "#fff", borderRadius: 12, padding: 14, marginBottom: 10, border: "1px solid #F0EDE8" }}>
                  ...
                  {d.spots.filter(s => s.budget > 0).map(sp => (
                    <div key={sp.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0", borderTop: "1px solid #F5F5F3" }}>
                      ...
                      <span style={{ fontSize: 13, color: "#059669", fontWeight: 600, opacity: sp.done ? 0.45 : 1 }}>¥{Number(sp.budget).toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              );
      ...
      {modal === "addSpot" && (
        <SpotForm title={`新增景點 · Day ${activeDay + 1}`}
          initial={{ time: "", name: "", cat: "景點", note: "", transit: "", budget: 0 }}
...
function HomeScreen({ trips, onSelect, onAddTrip, onDelete, onShare, onRefresh, onJoin, syncStatus = {}, coverTheme, onChangeCover }) {
  ...
  return (
    <div style={{ fontFamily: "'Hiragino Sans','Noto Sans TC',sans-serif", minHeight: "100vh", background: "#FAFAF8" }}>
      ...
      {/* ── Hero cover ── */}
      <div style={{ background: theme.bg, padding: "52px 24px 36px", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: -40, right: -40, width: 200, height: 200, borderRadius: "50%", background: "rgba(255,255,255,0.06)" }} />
        {/* Change cover button */}
        <button onClick={() => setShowCoverPicker(true)}
          style={{ position: "absolute", top: 16, right: 16, display: "flex", alignItems: "center", gap: 4, background: "rgba(255,255,255,0.18)", border: "1px solid rgba(255,255,255,0.35)", borderRadius: 20, padding: "5px 11px", color: "#fff", fontSize: 11, fontWeight: 600, cursor: "pointer", backdropFilter: "blur(4px)" }}>
          🎨 更換封面
        </button>
        <div style={{ fontSize: 11, color: "rgba(255,255,255,0.55)", letterSpacing: "0.25em", marginBottom: 6, fontFamily: "serif" }}>旅のきろく</div>
        <div style={{ fontSize: 36, fontWeight: 800, color: "#fff", letterSpacing: "-1px" }}>日本旅遊</div>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.7)", marginTop: 3 }}>行程規劃手帳</div>
      </div>

      {/* ── Trip list ── */}
      <div style={{ padding: "18px 16px" }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#9CA3AF", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 10 }}>我的旅程</div>
        {trips.map(t => {
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
          <button onClick={onAddTrip}
            style={{ flex: 1, display: "flex", alignItems: "center", gap: 6, justifyContent: "center", padding: 12, background: "#EFF6FF", color: "#2D6BE4", border: "1.5px dashed #BFDBFE", borderRadius: 14, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
            <Icon name="plus" size={16} color="#2D6BE4" /> 新增
          </button>
          <button onClick={onJoin}
            style={{ flex: 1, display: "flex", alignItems: "center", gap: 6, justifyContent: "center", padding: 12, background: "#F0FDF4", color: "#059669", border: "1.5px dashed #BBF7D0", borderRadius: 14, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
            🔗 輸入邀請碼
          </button>
        </div>
      </div>
          ...
          <div style={{ background: "#fff", borderRadius: 20, padding: "24px 20px", width: "100%", maxWidth: 340, textAlign: "center" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setConfirmDelete(null)}
                style={{ flex: 1, padding: "11px", background: "#F3F4F6", border: "none", borderRadius: 10, fontSize: 14, fontWeight: 600, color: "#374151", cursor: "pointer" }}>取消</button>
              <button onClick={() => { onDelete(confirmDelete); setConfirmDelete(null); }}
                style={{ flex: 1, padding: "11px", background: "#EF4444", border: "none", borderRadius: 10, fontSize: 14, fontWeight: 700, color: "#fff", cursor: "pointer" }}>刪除</button>
            </div>
          </div>
        </div>
      )}
...
// ─── Styles ───────────────────────────────────────────────────────────────────
const S = {
  app:         { fontFamily: "'Hiragino Sans','Noto Sans TC',sans-serif", background: "#FAFAF8", minHeight: "100vh", maxWidth: 430, margin: "0 auto" },
  header:      { display: "flex", alignItems: "center", gap: 10, padding: "14px 16px 10px", borderBottom: "1px solid #F0EDE8", background: "#fff", position: "sticky", top: 0, zIndex: 10 },
  ...
  dayTabs: { display: "flex", background: "#fff", borderBottom: "1px solid #F0EDE8", padding: "0" },
  dayTab:  { flex: 1, display: "flex", flexDirection: "column", alignItems: "center", padding: "9px 4px", background: "none", border: "none", borderBottom: "2px solid transparent", cursor: "pointer", gap: 2, minWidth: 0 },
  dayTabOn:{ borderBottomColor: "#2D6BE4" },

  dateBadgeBtn: { fontSize: 11, color: "#2D6BE4", background: "#EFF6FF", border: "1px solid #BFDBFE", padding: "3px 9px", borderRadius: 20, cursor: "pointer", whiteSpace: "nowrap", fontWeight: 600 },

  nav:   { display: "flex", background: "#fff", borderBottom: "1px solid #F0EDE8", padding: "4px 8px" },
  navBtn:{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "8px 4px", background: "none", border: "none", cursor: "pointer", borderRadius: 10 },
  ...
  // spot card
  card:    { display: "flex", alignItems: "center", gap: 8, background: "#fff", borderRadius: 14, padding: "11px 10px 11px 8px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", border: "1px solid #F0EDE8", cursor: "grab", transition: "opacity 0.2s", userSelect: "none" },
  ...
  rBtn:    { width: 28, height: 28, borderRadius: "50%", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 },

  addBtn:  { display: "flex", alignItems: "center", gap: 6, justifyContent: "center", width: "100%", marginTop: 12, padding: 13, background: "#2D6BE4", color: "#fff", border: "none", borderRadius: 12, fontSize: 14, fontWeight: 600, cursor: "pointer" },
  ...
  sheet:      { background: "#fff", borderRadius: "20px 20px 0 0", padding: "16px 20px 36px", width: "100%", maxWidth: 430, margin: "0 auto", maxHeight: "92vh", overflowY: "auto" },
  ...
  inp:        { width: "100%", padding: "10px 12px", border: "1px solid #E5E7EB", borderRadius: 10, fontSize: 14, color: "#1A1A1A", outline: "none", boxSizing: "border-box", background: "#FAFAF8" },
  confirmBtn: { width: "100%", marginTop: 18, padding: 13, background: "#2D6BE4", color: "#fff", border: "none", borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: "pointer" },
};
